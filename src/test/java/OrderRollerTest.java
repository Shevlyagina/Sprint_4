...
public class OrderRollerTest {
    ...
    public void testOrderOfRoller() {
        ...
        //заполнение всех полей заказа, кнопка "Заказать" и подтверждение "ДА"
        orderPageOrderData.fillSecondPageFields(data);
        //текст появившейся в результате позитивного заказа
        String result = orderPageOrderData.successModalGetText();
        System.out.println(result);
        //заведение переменной в которой сохраняется новые данные
        assertThat(result, CoreMatchers.startsWith("Хотите оформить заказ?"));
    }
